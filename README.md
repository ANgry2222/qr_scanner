qr_scanner - проект для сбора данных о покупках при помощи сканирования QR-кодов на чеках.

# Собираемые данные

-   Сумма покупки
-   Дата совершения покупки
-   Время совершения покупки
# Запуск приложения
Приложение размещено на GitHub Pages, доступ к нему можно получить по ссылке [angry2222.github.io/qr_scanner](https://angry2222.github.io/qr_scanner/).

# Работа с приложением

### Получение данных с помощью сканера
Запустите сканирование кликом по кнопке 'GO'. Наведите камеру на QR-код на чеке и зафиксируйте до появления анимации. При необходимости отдалите или приблизьте камеру. _Плохо пропечатанные и выцветшие QR-коды считываются гораздо хуже_.

### Ручной ввод данных

Приложение позволяет добавить данные о покупке для подсчёта затрат вручную с помощью кнопки '+'. Поле суммы является обязательным для заполнения, поле даты и времени покупки на данный момент являются опциональными.

### Очистка списка результатов

Чтобы полностью очистить список необходимо кликнуть по кнопке с изображением корзины.

### Удаление одиночного элемента

Чтобы удалить определённый элемент из списка нужно перетащить его справа налево до смены цвета индикатора удаления на красный, затем перестать удерживать элемент.

![Пример использования GIF](https://i.ibb.co/mC23HPsC/exa.gif)
### Экспорт данных

Полученный список можно скачать в формате '.txt'. Файл будет содержать все введённые пользователем данные. Данные о покупках разделены табуляцией для удобного импорта в MS Excel и подобные программы.

